...
| 功能 | 说明 | 状态 |
|------|------|------|
| **订单/物流查询** | 检测订单号/快递单号 → 查真实 MySQL（第二项目 copilot 库 30万订单 + tracking 轨迹），MySQL 不可达自动降级 mock | ✅ |

> 📌 订单/物流数据：复用第二项目 copilot MySQL（orders 30万 + tracking 24.6万轨迹，只读账号 `agent_ro`）。
> 演示单号：`ORD000000001`（已签收，韵达 `YD000000638365`）/ 物流轨迹 `SF1234567890`。配置见 `src/config.py` §订单数据库。
...
> 📖 **[docs/启动与测试.md](docs/启动与测试.md)** — 怎么启动（本地/VM docker）、怎么测试（订单/物流/RAG/评测）、怎么切换环境、常见问题。
